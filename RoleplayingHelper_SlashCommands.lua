...
-------------------------------------------------------------------------
	    elseif  ( command == "dwarf" ) or
			...
			if PlaceInArray( RPCONFIG.traits, string.upper(command) ) then
				...
				DEFAULT_CHAT_FRAME:AddMessage( "You're no longer roleplaying like a " .. string.lower(command) .. ".", 0.4, 1.0, 0.6  )
			else
				table.insert( RPCONFIG.traits, string.upper(command) )
				DEFAULT_CHAT_FRAME:AddMessage( "You're now roleplaying like a " .. string.lower(command) .. ".", 0.4, 1.0, 0.6  )
			end
   		...
   		elseif ( command == "night" ) then
			if PlaceInArray( RPCONFIG.traits, "NIGHTELF" ) then
				...
				DEFAULT_CHAT_FRAME:AddMessage( "You're no longer roleplaying like a night elf.", 0.4, 1.0, 0.6  )
			else
				table.insert( RPCONFIG.traits, "NIGHTELF" )
				DEFAULT_CHAT_FRAME:AddMessage( "You're now roleplaying like a night elf.", 0.4, 1.0, 0.6  )
			end
		elseif ( command == "high" ) then
			if PlaceInArray( RPCONFIG.traits, "HIGHELF" ) then
				table.remove( RPCONFIG.traits, PlaceInArray(RPCONFIG.traits,"HIGHELF") )
				DEFAULT_CHAT_FRAME:AddMessage( "You're no longer roleplaying like a high elf.", 0.4, 1.0, 0.6  )
			else
				table.insert( RPCONFIG.traits, "HIGHELF" )
				DEFAULT_CHAT_FRAME:AddMessage( "You're now roleplaying like a high elf.", 0.4, 1.0, 0.6  )
			end
		elseif ( command == "ork" ) then
			if PlaceInArray( RPCONFIG.traits, "ORC" ) then
				table.remove( RPCONFIG.traits, PlaceInArray(RPCONFIG.traits,"ORC") )
				DEFAULT_CHAT_FRAME:AddMessage( "You're no longer roleplaying like a gigachad ork.", 0.4, 1.0, 0.6  )
			else
				table.insert( RPCONFIG.traits, "ORC" )
				DEFAULT_CHAT_FRAME:AddMessage( "You're now roleplaying like a gigachad ork.", 0.4, 1.0, 0.6  )
			end
			DEFAULT_CHAT_FRAME:AddMessage( "Your RP traits are: ".. table.concat(RPCONFIG.traits, ", "), 0.4, 1.0, 0.6 )
